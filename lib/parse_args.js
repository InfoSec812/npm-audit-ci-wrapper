...
const options = [
  ...
  },
  {
    ...
    short: 'w',
    type: 'list,string',
    description: 'Whitelist the given dependency at the specified version (Can be specified multiple times).',
    example: "'npm-audit-ci-wrapper -w https-proxy-agent:1.0.0' or 'npm-audit-ci-wrapper --whitelist=https-proxy-agent:1.0.0'"
  }
];
...
function parse_args(cli_args = process.argv) {
  ...
  let whitelist = [];
  if (args.options.hasOwnProperty('whitelist') && (args.options.whitelist.length > 0) ) {
